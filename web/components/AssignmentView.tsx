...
import type { ItemAttachment } from "@/lib/types";
 ...
 *
 * The stages are real — each one is reported as it starts — and the
 ...
 * server, which holds the API key. A link leaves
...
function Attachments({ items, domain }: { items: ItemAttachment[]; domain: string }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      ...
      {items.map((at) => {
        // Every attachment opens on Schoology. A student-scoped API key can
        // list a teacher's files but cannot fetch the bytes behind one, so
        // there is nothing to preview inline — and a viewer that renders an
        // empty frame is worse than a link that works.
        ...
        return (
          <div key={at.url} style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            <button
              ...
              onClick={() => window.open(away, "_blank", "noopener,noreferrer")}
              style={{
    ...
    <div className="scroll centered" style={{ paddingBottom: 32 }}>
      <div className="col" style={{ maxWidth: 1300, gap: 20 }}>
        ...
        {/* What the teacher actually wrote, structure and all. Markup when the
            sync kept it; otherwise the plain text, whose line breaks are
