...
Playwright-controlled Schoology session, so it cannot be packaged as a static
web export without removing core behavior.

On a physical phone:

- The Next.js portal and `scraper/` run on a trusted computer or private host.
- The phone talks only to the configured portal URL; the portal talks to the
  scraper on its own localhost.
- Board state is cached in the phone's WebView storage after it has loaded.
- Sync, submission, message sending, assessment overlays, file proxying, and
  server-backed tutor features need the host to be online. The phone does not
  run Chrome/Playwright or hold a Schoology cookie itself.
...
Start the services on the host computer in separate terminals:
...
deployment/update service) and keep the Next.js/scraper services behind an
